  ...
  useEffect(() => {
    if (data && isSuccess) {
      if (
        ...
        new Date(`${data.startDate}T${data.startTime}`).toISOString() !== new Date(invoiceData.timeStart).toISOString()
      ) {
        updateInvoiceData(data.startLocation, data.endLocation, new Date(`${data.startDate}T${data.startTime}`));
      }
    }
