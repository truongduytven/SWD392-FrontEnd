...
function RatingDetailLayout({tripID, tripRatingDetails, isLoading, error }: RatingTabProps) {
  ...
  return (
    ...
    {isLoading ? (
        ...
        <div className='flex gap-3'>
          {buttons.map((button) => (
            <Button
              ...
              variant={selectedValue === button.value.toString() ? 'default' : 'outline'} 
      ...
      {tripRatingDetails.feedbacks.map((feedback, index) => (
            <RatingDetail feedback={feedback} />
        ))}
    </div>
    ) : (
